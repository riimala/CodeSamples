		...
		try {
			...
			rdbtnRead.addMouseListener(new MouseAdapter() {
				...
				public void mouseClicked(MouseEvent e) {
					...
					try {
						...
						int count = records.Count(t, conn2);

						if ( count == 0) {
							infoBox("No Records", "Record check");
							return;
						}												
			...
			rdbtnDel.addMouseListener(new MouseAdapter() {
				...
				public void mouseClicked(MouseEvent e) {
					...
					String s = JOptionPane.showInputDialog("Which id need to be deleted?");
			...
			JRadioButton rdbtnDropTable = new JRadioButton("Drop Table");
			rdbtnDropTable.addMouseListener(new MouseAdapter() {
			@Override
			public void mouseClicked(MouseEvent e) {
				
				int result = JOptionPane.showConfirmDialog(frmTaskariAdmin,"Sure, you want to wipe out whole table?", "Critical Confirmation",
			            ...
			            if(result == JOptionPane.YES_OPTION){
							...
							try {
								r.DropTable(t, conn2);
								infoBox("Table destroyed", "Deletion Operation");
							} catch (SQLException e1) {
								infoBox("Table destroying failed", "Failure");
								e1.printStackTrace();								
							}			            			          			            
			            }else if (result == JOptionPane.NO_OPTION){
			            	infoBox("Table removal cancelled", "Deletion Operation");
			            }				
			            rdbtnDropTable.setSelected(false);            
			}
		});
			
			rdbtnDropTable.setBounds(178, 159, 120, 23);
			frmTaskariAdmin.getContentPane().add(rdbtnDropTable);
